...
sound_file_path = os.path.join(base_path, "sound", "clash2.mp3")
LINE_OUT_SOUND = pg.mixer.Sound(sound_file_path)
...
def main():
    ...
    big_font = pg.font.SysFont('verdana', 20, True)
...
def drawInfo(points, level):
    pointsSurf = basic_font.render(f'Зерокойны: {points}', True, txt_color)
    pointsRect = pointsSurf.get_rect()
    pointsRect.topleft = (window_w - 560, 180)
    display_surf.blit(pointsSurf, pointsRect)

    levelSurf = basic_font.render(f'Уровень: {level}', True, txt_color)
    levelRect = levelSurf.get_rect()
    levelRect.topleft = (window_w - 560, 250)
    display_surf.blit(levelSurf, levelRect)

    pausebSurf = basic_font.render('Вращение: ↑', True, info_color)
    pausebRect = pausebSurf.get_rect()
    pausebRect.topleft = (window_w - 560, 390)
    display_surf.blit(pausebSurf, pausebRect)

    pausebSurf = basic_font.render('Сброс: пробел', True, info_color)
    pausebRect = pausebSurf.get_rect()
    pausebRect.topleft = (window_w - 560, 420)
    display_surf.blit(pausebSurf, pausebRect)

    escbSurf = basic_font.render('Выход: Esc', True, info_color)
    escbRect = escbSurf.get_rect()
    escbRect.topleft = (window_w - 560, 450)
